...
class Program extends EventEmitter {
  ...
  onUserAction(namespaceAndAction, handler) {
    this.userActionHandlers = this.userActionHandlers || {};
    this.userActionHandlers[namespaceAndAction] = this.userActionHandlers[namespaceAndAction] || [];
    this.userActionHandlers[namespaceAndAction].push(handler);
  ...
  userAction({ namespace, action, payload }) {
    for (const [namespaceAndAction, handlers] of Object.entries(this.userActionHandlers || {})) {
      const [_namespace, _action] = namespaceAndAction.split('/');
      if (!_namespace || _namespace == '*' || (_namespace == namespace && (!_action || _action == action))) {
        handlers.forEach(handler => handler({ namespace, action, payload }));
      }
    }
  }
  ...
  showNotification(
    ...
    if (originDevice) {
      if (omitDeviceName) {
        _title = title;
        _mainTitle = title || this.network.name();
      } else if (!title) {
        _title = originDevice;
        _mainTitle = this.network.name() || originDevice;
      } else {
        _title = `${originDevice} · ${title}`;
        _mainTitle = `${this.network.name() || originDevice} · ${title}`;
      }
    } else {
  ...
  periodicNotification(checkCallback, executeCallback) {
    ...
    setTimeout(() => {
      dmt.loop(() => {
        const result = checkCallback(this);

        if (result && (!reportedAt || Date.now() - reportedAt >= STANDOFF_INTERVAL)) {
          executeCallback({ program: this, result });
